...
> ⚠️ **The approval *prompt* is not yet wired end-to-end.** `ApprovalGate::init_global` is not called in production, `DomainEvent::ApprovalRequested` is bridged to no socket, and there is no general approval-toast UI — so `try_global()` is `None`, the harness skips the intercept, and `Prompt`-class calls currently **run unprompted**. Read-only blocking, path hardening, structural guards, and classification are live; the human prompt depends on building the gate surface (install + event bridge + toast — the #1339 frontend). Full access is shipped as documented full-trust (not sandboxed).
