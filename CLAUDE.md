...
> ⚠️ **The approval *prompt* is not yet usable end-to-end.** `ApprovalGate::init_global` runs only behind `OPENHUMAN_APPROVAL_GATE=1` (off by default, `jsonrpc.rs`), so by default `try_global()` is `None`, the harness skips the intercept, and `Prompt`-class calls **run unprompted**. Even with the flag on there is no answer path: `DomainEvent::ApprovalRequested` is published but bridged nowhere, no prompt UI exists, and a chat reply hits the web channel's "newer request aborts the in-flight turn" path (`channels/providers/web.rs`), cancelling the parked approval instead of answering it — so it parks to the 10-min TTL → Deny. Read-only blocking, path hardening, structural guards, and classification **are** live; the chat-native prompt (planned: surface the question as a thread message; route a `yes`/`no` reply to `approval_decide`; any other text cancels the turn and starts fresh) is the remaining build. Full access ships as documented full-trust (not sandboxed).
