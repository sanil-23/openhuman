...
use super::{CloudEmbedder, Embedder, OllamaEmbedder};
    ...
    match (
        ...
        _ => {
            ...
            if config.local_ai.use_local_for_embeddings() {
                ...
                // Default: cloud (OpenHuman backend / Voyage `voyage-3.5`, 1024 dims).
                // Matches the main embeddings path so a fresh install needs zero
                // local Ollama setup. Auth failures surface as embed-call errors
                // so ingest's existing retry-with-backoff logic handles them.
                log::debug!(
...
mod tests {
    ...
    #[test]
    fn unset_endpoint_falls_back_to_cloud() {
        let (_tmp, mut cfg) = test_config();
    ...
    #[test]
    fn local_ai_usage_off_falls_back_to_cloud() {
        // runtime_enabled=true but usage.embeddings=false → still cloud.
        let (_tmp, mut cfg) = test_config();
        cfg.memory_tree.embedding_endpoint = None;
        cfg.memory_tree.embedding_model = None;
        cfg.local_ai.runtime_enabled = true;
