...
async fn approval_rpc_decision_paths_persist_always_allow_and_recent_audit() {
    ...
    let gate = ApprovalGate::init_global(config.clone(), "session-approval-raw-e2e-session");
    ...
    assert_eq!(gate.session_id(), "session-approval-raw-e2e-session");

    let second_init =
        ApprovalGate::init_global(Config::default(), "session-ignored-second-session");
    assert_eq!(second_init.session_id(), "session-approval-raw-e2e-session");
